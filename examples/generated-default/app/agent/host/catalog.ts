...
export function buildFrontendToolDescriptors(
  ...
  // This app projects the surface in DIRECT mode only — one tool per
  // capability — so an unmapped wire name is always a capability that cannot
  // be audited under its canonical id, and it is withheld rather than offered
  // under a guessed one.
  ...
  for (const tool of toolset.tools()) {
    const canonicalId = wireNames.get(tool.name);
