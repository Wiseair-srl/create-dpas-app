...
**The agent path and the human path are one implementation.** The toolbar button and the assistant call the same oRPC procedure. The assistant's filter change and the user's filter change go through the same setter, so the URL moves either way and the view the agent produced is one you can bookmark and share. There is no second, quieter code path to keep in agreement — or to attack. That extends to the client cache: an agent write invalidates it exactly as a click does, and because a write can land in three different moments it takes three triggers to say one thing — the surface subscription for what runs in the browser, the stream consumer for what runs in the model loop, and the approval decision for a gated write, which executes only after the human says yes, when the stream that asked for it has long since closed. Wire only some of them and agent writes refresh the screen *sometimes*, which reads as a flaky UI rather than a missing trigger.
