...
| `-y, --yes` | flag | off | Accept every default; no prompts. For CI and scripts |
...
| `--install` / `--no-install` | flag | install | `--no-install` prints the install command instead |
| `--git` / `--no-git` | flag | init | `--git` initializes a repo on `main` and makes one unsigned initial commit |
| `--example <name>` | `default` | `default` | Template to generate |
| `-h, --help` | flag | | Print help and exit 0 |
| `-v, --version` | flag | | Print the CLI version and exit 0 |
