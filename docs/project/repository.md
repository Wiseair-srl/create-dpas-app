...
Review a diff in either artifact like an API diff, because it is one: a changed description is a changed prompt, and a capability that gains a surface has gained an audience. The domain gate reports each runtime policy's declared scope and matching candidate capabilities in the `POLICIES` column. It does **not** evaluate the policy: actor, input, context, and concrete surface still decide the runtime verdict. A `0` in its APPROVAL column is metadata, not a reachability claim.
