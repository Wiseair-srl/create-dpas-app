...
`claude-sonnet-4.5`, which is not a valid id there, and the run dies with
OpenRouter's *"No endpoints found that support tool use"*, which points at the
tools rather than at the real cause. The app prefixes the gateway for you
