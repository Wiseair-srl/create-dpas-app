...
The directive requires a demo identity system (`analyst` / `controller`) resolved
...
HMAC-SHA256 (`AUTH_SECRET`, defaulted for local dev). `server/auth.ts`
verifies the signature on every request and derives the actor; a missing or
invalid cookie yields the default `controller` demo user so the app works on
...
This is explicitly a demo: the docs point at the seam (`server/auth.ts`)
